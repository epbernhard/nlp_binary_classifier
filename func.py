...
def compile_and_fit(model, batch_size, epochs, optimizer, X_train, Y_train, X_test, Y_test):
    ...
    history = model.fit(X_train, Y_train,epochs = epochs,verbose = True,validation_data = (X_test, Y_test),batch_size = batch_size,callbacks = e_s)
    tf.keras.backend.clear_session()
    
    return history


def perform_cleaning(df_path1, df_path2, key1, key2, df_path):
  ...
  # remove empty rows
  df_comments_efftreat = df_comments_efftreat[(df_comments_efftreat['comments_clean'] != '').values]

  # Save the labels
  df_comments_efftreat.to_csv(df_path, columns = ['comments_clean', 'labels'], index = False)

def clean_comments(comments):
    """Clean the comments (to expand)"""
    ...
    with tqdm(total=len(comments)) as pbar:
        for i, comment_i in enumerate(comments):
            ...
            try:
                ...
                err_msg = "It looks like the string '{}' could not be handled.".format(comment_i)
...
def perform_pretreat(df):
  ...
  axs.tick_params(axis='both', labelcolor='k', labelsize = 18, width = 1, size = 20, which = 'major', direction = 'inout')
  axs.tick_params(axis='both', width = 1, size = 10, which = 'minor', direction = 'inout')
  fig.savefig('./plots/data_balance_full_sample.pdf')
  plt.close(fig)

  print('The train data are imbalanced at a level of %i '%((df_balance['comments_clean'].values/len(df)*100.)[0]) +
        'and %i percent for class 0 and class 1, respectively.'%((df_balance['comments_clean'].values/len(df)*100.)[1]))
  ...
  df_usefull = df.loc[df['labels'] == 1, 'comments_clean'].tolist()
  df_useless = df.loc[df['labels'] == 0, 'comments_clean'].tolist()

  n_word = 30 #Numbers of top words to look at
  top_usefull = pd.DataFrame(Counter(' '.join(df_usefull).split()).most_common(n_word), columns = ['word', 'count'])
  ...
  fig.subplots_adjust(hspace = 0.0, wspace=0., left  = 0.16, right = 0.97, bottom = 0.28, top = 0.98)
  ...
  ax1.tick_params(axis='both', labelcolor='k', labelsize = 18, width = 1, size = 20, which = 'major', direction = 'inout')
  ax1.tick_params(axis='both', width = 1, size = 10, which = 'minor', direction = 'inout')
  ...
  df_word_counter_all = top_usefull.merge(top_useless, left_on = 'word', right_on = 'word', suffixes=('_usefull', '_useless'))
  ...
  frac_word_usefull = df_word_counter_all['count_usefull'].values/sum(df_word_counter_all['count_usefull'].values)
  ...
  o = np.where((word_power > -0.1) & (word_power < 0.1))[0]
  ax2.plot(df_word_counter_all['word'].values[o], word_power[o], 'o', mfc = 'None', mew = 1, mec = 'k')
  o = np.where((word_power > 0.1))[0]
  ax2.plot(df_word_counter_all['word'].values[o], word_power[o], 'o', mfc = '#87b9e8', mew = 1, mec = 'k')
  o = np.where((word_power < -0.1))[0]
  ax2.plot(df_word_counter_all['word'].values[o], word_power[o], 'o', mfc = '#e8a787', mew = 1, mec = 'k')

  ax2.plot(df_word_counter_all['word'].values, np.zeros(len(df_word_counter_all)), 'k--', lw = 1)
  ax2.plot(df_word_counter_all['word'].values, np.zeros(len(df_word_counter_all))+0.1, 'k--', lw = 1)
  ax2.plot(df_word_counter_all['word'].values, np.zeros(len(df_word_counter_all))-0.1, 'k--', lw = 1)
  ax2.set_ylim([-0.3, 0.3])
  ax2.set_ylabel('word positivity (fraction)', fontproperties = font0, fontsize = 16)
  ax2.tick_params(axis='both', labelcolor='k', labelsize = 18, width = 1, size = 20, which = 'major', direction = 'inout')
  ...
  y, x = np.histogram(com_length, bins = 100)
  
  fig, ax1 = plt.subplots(1, 1, figsize = (11, 7))
  fig.subplots_adjust(hspace = 0.0, wspace=0., left  = 0.16, right = 0.97, bottom = 0.12, top = 0.98)
  
  y_cumsum = y.cumsum()/y.cumsum().max()
  # To the nearest 10:
  comments_length = int(np.round(x[0:-1][y_cumsum >= frac_words][0]/10.)*10)

  dx = (x[1:] - x[:-1])/2.
  ax1.bar(x[0:-1] + dx, y_cumsum, width = dx*2., edgecolor = 'k', 
          linewidth = 1, color = ['#87b9e8'], alpha = 0.4)
  ax1.plot([0., x[-1]], [frac_words, frac_words], 'k--', lw = 2)
  ...
  frac_vocab = 0.9 # We capture 90% of the most important words (as measured from the number of times it appears).
  vocab = Counter(' '.join(df['comments_clean']).split())
  n_hit_unique = np.unique([val for val in vocab.values()])

  y, x = np.histogram(n_hit_unique, bins = 200)
  dx = (x[1:] - x[:-1])/2.
  
  n_hit_lim = int(np.interp(frac_vocab, y[::-1].cumsum()/y[::-1].cumsum().max(), (x[0:-1] + dx)[::-1]))

  fig, ax1 = plt.subplots(1, 1, figsize = (11, 7))
  fig.subplots_adjust(hspace = 0.0, wspace=0., left  = 0.16, right = 0.97, bottom = 0.12, top = 0.98)
  ax1.bar(x[0:-1] + dx, y/y.max(), width = dx*2., edgecolor = 'k', 
          linewidth = 1, color = ['#87b9e8'], alpha = 0.4)
