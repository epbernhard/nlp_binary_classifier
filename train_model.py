...
if USE_SAVED_EMBEDDING is not True:
    ...
    # and their labels (either 1 or 0 for useful or useless)
    # The aim is to use NLP to decide whether a comment is useful.

    # Shuffle the dataframe (saved the shuffle one for reproductability in Dev)
    # df_clean = df_clean.sample(frac=1).reset_index(drop=True)
    # df_clean.to_csv('./data/efftreat_clean_label.csv', index = False)
    # pdb.set_trace()

    # We split into Train and Test samples (25%), and we pre-treat on the train sample only.
    Ntrain = int(0.75 * len(df_clean))
    ...
    with open('./vocabulary.txt', 'w') as f:
        f.write(json.dumps(word_in_vocab))
...
history = func.compile_and_fit(model, BATCH_SIZE, EPOCHS, sgd, X_train, Y_train, X_test, Y_test)
...
F1_score = int(func.fscore(1, precision, recall))
F0p5_score = int(func.fscore(0.5, precision, recall))
...
with open('./histories/model_'+version+'.txt', 'w') as f:
    ...
    f.write('The loss is %f.'%(loss)+'\n')
    f.write('The F1-score and F0.5-score are %i and %i percents, respectively.\n'%(F1_score, F0p5_score))
    f.write('--------------')
